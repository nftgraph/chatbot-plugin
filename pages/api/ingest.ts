...
import { fileConsumer, formidablePromise } from '@/utils/server/formidable';
import { createPineconeIndex } from '@/utils/server/pinecone-client';
import { PINECONE_NAME_SPACE } from '@/config/pinecone';
import extractTextFromFile from '@/utils/server/extractTextFromFiles';
...
export default async function handler(
  ...
  try {
    ...
    const embeddings = new OpenAIEmbeddings({
      openAIApiKey: openaiApiKey ? openaiApiKey : process.env.OPENAI_API_KEY,
    });
